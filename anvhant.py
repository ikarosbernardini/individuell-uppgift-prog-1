...
class Användarhantering:
        ...
        for anv in self.användare:
            if anv.namn == nuv_namn:
                ...
                if any(a.namn == nytt_namn for a in self.användare):
                    print("\nAnvändarnamnet är upptaget, vänligen test med ett annat. ")
                    self.återgå()
                    return
        ...
        while self.felaktiga_försök > 0: # så länge användaren har försök kvar att logga in.
            ...
            for anv in self.användare: # loopar igenom alla användare i systemet
                ...
                if self.felaktiga_försök == 0:
                    print("\nFör många felaktiga försök. Programmet avslutas.") # felmeddelande om användaren har gjort för många felaktiga försök.
                    exit()
                else:
                    print(
                        f"\nFelaktigt användarnamn eller lösenord. Du har {self.felaktiga_försök} försök kvar, Försök igen" # informerar användaren hur många försök den har kvar.
                    ) 
    ...
    def __init__(self, filväg: str) -> None:
        ...
        self.val: int = 0
        self.inloggad_anv: A | None = None  # håller reda på vilken användare som är inloggad. användimport json # importerar json för att kunna läsa och skriva till .json filen
import hashlib # importerar hashlib för att kunna hasha lösenord
from anv import Användare as A # importerar klassen Användare från anv.py
    ...
    def byt_namn(self):
        ...
        for anv in self.användare: # för varje användare i listan så kollar vi om användarnamnet matchar den inmatade användaren.
            if anv.namn == nuv_namn: # om användarnamnet matchar den inmatade användaren
                lösen = input("Ange lösenord för att bekräfta indentitet: ")
